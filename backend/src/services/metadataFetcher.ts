...
export async function fetchBilibiliMetadata(bvid: string): Promise<VideoMetadata | null> {
  try {
    // 先尝试短链解析
    let finalBvid = bvid
    if (bvid.length < 12) {
 ...
 * 备用方案: 通过网页爬取 (如果API失败)
 */
export async function fetchBilibiliMetadataFromPage(bvid: string): Promise<VideoMetadata | null> {
  try {
