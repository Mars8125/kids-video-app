...
let bot = null;
export async function initTelegramBot(token, webhookUrl) {
    ...
    try {
        ...
        if (e.response?.body?.error_code !== 404)
            console.warn('清理 webhook 失败:', e.message);
    }
    // 设置新 webhook
    ...
    bot.onText(/\/start/, async (msg) => {
        await bot.sendMessage(msg.chat.id, `🎬 小学生视频App - 管理后台\n\n` +
    ...
    bot.onText(/\/help/, async (msg) => {
        await bot.sendMessage(msg.chat.id, `📖 使用说明\n\n` +
            `1. 复制视频链接\n` +
            `2. 直接粘贴发送给我\n` +
            `3. 我会自动解析并添加到App\n\n` +
            `💡 小技巧:\n` +
    ...
    bot.onText(/\/stats/, async (msg) => {
        ...
        await bot.sendMessage(msg.chat.id, `📊 统计概览\n\n` +
            `总视频数: ${total}\n` +
            `B站: ${bilibili}\n` +
            `抖音: ${douyin}`);
    });
